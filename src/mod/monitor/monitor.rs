...
pub fn load_entry() {
    ...
    let mut f =
        std::fs::File::open(inst_file).expect(format!("Can not open '{}'", inst_file).as_str());
    f.read_to_end(&mut buf).unwrap();
    unsafe {
        ptr::copy_nonoverlapping(
            buf.as_ptr(),
            (DRAM.as_mut_ptr() as *mut u8).offset((ENTRY_START & 0x1F_FF_FF_FF) as isize),
            buf.len(),
        );
...
mod test {
    ...
    fn write() {
        unsafe {
            ...
            memcpy(
                buf.as_ptr(),
                (DRAM.as_mut_ptr() as *mut u8).offset(0x1fc00000),
                buf.len() as isize,
            );
